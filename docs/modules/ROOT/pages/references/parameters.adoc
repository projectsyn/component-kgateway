...
* Resources are created in the namespace configured in `namespace` by default, overridable via `metadata.namespace`
* Values are deep-merged using https://syn.tools/commodore/reference/api.html#_makemergeable[`com.makeMergeable()`]
====
...
Whether to create a ClusterRole aggregated to `cluster-reader` that grants read access to Gateway API and kgateway CRDs.
